...
function ViewHeader({ meetingId }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const location = useLocation();

  const tabs = [
    { id: "details", label: "Meeting Details", path: `/meetings/${meetingId}/detailsView` },
    { id: "notes", label: "Notes", path: `/meetings/${meetingId}/notesView` },
    { id: "tasks", label: "Action Items", path: `/meetings/${meetingId}/tasksView` },
  ];
    ...
    <div className="relative" style={{ top: "-25px" }}>
      <header className="relative z-50 mx-auto px-6 py-10 flex justify-between items-center text-black" style={{ maxWidth: "1200px", color: "#111" }}>
        ...
        <div className="md:hidden relative z-60 ">
          <button className="focus:outline-none focus:ring-0" onClick={() => setMenuOpen(!menuOpen)}>
            <svg xmlns="http://www.w3.org/2000/svg" height="32px" viewBox="0 -960 960 960" width="32px" className="fill-black">
      ...
      {menuOpen && (
        <div className="fixed top-0 right-0 h-full w-3/4 bg-white bg-opacity-80 z-70 flex flex-col items-center justify-center md:hidden transition-all duration-300 ease-in-out">
          <nav className="flex flex-col gap-8">
            {tabs.map((tab) => (
              <Link key={tab.id} to={tab.path} onClick={() => setMenuOpen(false)} className={`text-xl flex items-center gap-2 no-underline ${isActiveTab(tab.path) ? "font-bold" : ""}`}>
                {tab.label}
              </Link>
            ))}
          ...
          <button className="absolute top-6 right-6 text-3xl focus:outline-none focus:ring-0 bg-white" onClick={() => setMenuOpen(false)}>
            &times;
          </button>
        </div>
      )}
